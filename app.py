...
label_version = tk.Label(window, text = " Version : 1.0")
label_version.grid(row = 0, column = 0)

# 输入序号栏
var_entry = tk.StringVar()
entry = tk.Entry(window, width = 20, textvariable = var_entry)
...
listbox = tk.Listbox(window, height = 3, width = 20, listvariable = var_listbox)
listbox.insert("end", "暂无收藏股票序号     ")
listbox.grid(row = 3, column = 2, columnspan = 4)

# 文本框
text = tk.Text(window, height = 7, width = 30)
